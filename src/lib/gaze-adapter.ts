...
export default class GazeAdapter {
    ...
    private sampleCallback: ((sample: GazeSample) => void) | null = null;
    ...
    async calibrate(points: { x: number, y: number }[] = []): Promise<CalibrationResult> {
        ...
        if (errors.length === 0) {
            console.warn('[GazeAdapter] No validation errors collected');
            return {
                rmse: 0.1,
                rmse_pixels: 100,
                errors: [],
                validation_points: validationPoints.length,
                warning: 'No validation samples collected'
            };
        }
    ...
    destroy(): void {
        ...
        console.log('GazeAdapter destroyed');
    }
